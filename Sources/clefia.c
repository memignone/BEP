...
/*  Two different kinds of non-linear functions called S-boxes are used in CLEFIA.
 *  By combining these two S-boxes with different algebraic characteristics,
 * immunity against byte-oriented algebraic attacks is expected to be strong.
 */
...
unsigned char ClefiaMul2(unsigned char x) { /* multiplication over GF(2^8) (p(x) = '11d') */
		if(x & 0x80U) {
				x ^= 0x0eU;
		}
		return ((x << 1) | (x >> 7));
...
/* 
 *  CLEFIA employs Diffusion Switching Mechanism （DSM), which ensures immunity
 * against both differential and linear attacks with a fewer number of rounds,
 * so DSM can make blockciphers faster.
 *  CLEFIA employs a 4-branch Feistel structure to realize compactness and high speed.
 */
...
/* Generalized Feistel network (GFN) is a widely used design for encryption algorithm such as DES, IDEA and others. */
...
/*
 *  A simple linear operation called DoubleSwap is used in the key scheduling part.
 *  The DoubleSwap function is a bit operation taking a 128-bit value as an input,
 * then dividing the input into 4 parts and shuffling them to increase security.
 *  The DoubleSwap function can be used for rapidly generating round keys while
 * maintaining the efficiency of software and hardware implementation.
 */
...
/* CLEFIA employs whitenings at the beginning and the end of the data processing part
	because these partial whitenings provide enough key information (entropy) for the data processing part */
